...
import { EGameStatus, EGeneratorType, EModalComponents, IAppContext } from '../types';
...
vi.mock('../../utils/generators/symmetricGenerator', () => ({
    generateSymmetricBoard: vi.fn(() => {
        const board: TBoard = Array.from({ length: 9 }, () => Array(9).fill(1));
        const solution: TBoard = Array.from({ length: 9 }, () => Array(9).fill(2));
...
vi.mock('../../utils/generators/isomorphicGenerator', () => ({
    generateIsomorphicBoard: vi.fn(() => {
        const board: TBoard = Array.from({ length: 9 }, () => Array(9).fill(3));
        const solution: TBoard = Array.from({ length: 9 }, () => Array(9).fill(4));
...
vi.mock('../../utils/generators/technique', () => ({
    generateTechniqueBoard: vi.fn(() => {
        const board: TBoard = Array.from({ length: 9 }, () => Array(9).fill(5));
        const solution: TBoard = Array.from({ length: 9 }, () => Array(9).fill(6));
...
describe('startGame', () => {
    it('dispatches 7 actions in order', () => {
        ...
        startGame('easy' as TDifficultyKey)(dispatch, state);
        ...
        expect(types[3]).toBe(EGameInfoActionTypes.SetGameStatus);
        expect(dispatch.mock.calls[3][0].payload).toBe(EGameStatus.InProgress);
...
describe('leaveAfterWin', () => {
    it('dispatches 7 actions ending with setGameStatus(NotStarted)', () => {
        ...
        const last = dispatch.mock.calls[6][0];

        expect(last.type).toBe(EGameInfoActionTypes.SetGameStatus);
        expect(last.payload).toBe(EGameStatus.NotStarted);
    });
});
...
describe('setValueToBoard', () => {
    it('dispatches setBoard, setClickedCellValue, and pushToHistory', () => {
        ...
        expect(types[0]).toBe(EBoardsActionTypes.SetBoard);
        expect(types[1]).toBe(EClickedCellActionTypes.SetClickedCellValue);
        expect(dispatch.mock.calls[1][0].payload).toBe(5);
        expect(types[2]).toBe(EHistoryActionTypes.PushToHistory);
        expect(dispatch.mock.calls[2][0].payload).toEqual({ ...clickedCell, value: 5 });
    });
});
